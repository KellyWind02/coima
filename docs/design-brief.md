...
- **Out of scope（本 pass）**：策略中心、图表、投注、暗色主题切换、移动端极致体验。
...
- **Density**：**dense**（多列、多行）。
- **Critical fields（无点击可见）**：日期、场地、场次、马号、马名、独赢/位置赔率。
...
| 3 | Results table | 阅读与排序对比 | 可排序表 + 行选中 | 主视觉＝数据本身 |
...
1. 视觉方向：浅色冷灰 + 低饱和绿强调，是否 OK？若要「更赛马/更金融」，偏好哪边？
2. 产品名是否需要副标题（如「历史赛事与赔率检索」）？默认一句短说明。
3. 筛选条是否 **sticky** 贴顶？（桌面长表建议是）
4. 策略中心入口：MVP 完全隐藏 / 页头放禁用态链接？
